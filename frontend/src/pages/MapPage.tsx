...
interface SocialPost {
  ...
  awarenessCount?: number; // Number of people who raised awareness about this issue
}
...
const AnimatedFlightPath: React.FC<{ path: FlightPath; onClick?: () => void }> = ({ path, onClick }) => {
  ...
  useEffect(() => {
    ...
    // Vary speed based on distance for realism - ultra slow and buttery smooth
    const distance = Math.sqrt(
      Math.pow(path.to.lat - path.from.lat, 2) +
      Math.pow(path.to.lng - path.from.lng, 2)
    );
    const speed = 0.0015 + (distance * 0.00002); // Ultra slow paths
  ...
  const typeIcons = { funding: '💰', data: '📊', collaboration: '🤝' };
...
const PulseCircle: React.FC<{ location: { lat: number; lng: number }; type: string }> = ({ location, type }) => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setPulse(prev => (prev >= 300000 ? 50000 : prev + 2000)); // Slower
    }, 600); // Slower pulse
...
const MapPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    }, 30000); // Slower activities // New activity every 15 seconds
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between gap-8">
            ...
            <div className="flex items-center gap-2">
              <span className="text-xs text-gray-500 uppercase tracking-wide">Funding (MTD)</span>
              <span className="text-2xl font-bold text-emerald-600">${(networkStats.totalTransferred / 1000000).toFixed(1)}M</span>
            </div>

            {/* Projects with Trend */}
            <div className="flex items-center gap-2">
              <span className="text-xs text-gray-500 uppercase tracking-wide">Active Projects</span>
              <span className="text-2xl font-bold text-purple-600">{networkStats.projectsActive}</span>
            ...
            {/* Top Current Mission */}
            <div className="flex-1 flex items-center gap-3 bg-blue-50 px-4 py-2 rounded-lg border border-blue-100">
              <span className="text-xs text-blue-700 font-semibold uppercase tracking-wide">Top Mission</span>
              <span className="text-sm text-blue-900 font-medium truncate">Amazon Reforestation • $340K active</span>
            </div>

            {/* Voting Power - Moved to far right */}
            <div className="bg-arctic-50 px-4 py-2 rounded-lg border border-arctic-200">
              <div className="text-xs text-gray-600">Voting Power</div>
              <div className="text-xl font-bold text-arctic-600">{userVotingPower}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-4">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2">
            ...
            <div className="bg-white rounded-2xl shadow-2xl overflow-hidden relative border-2 border-gray-100" style={{ height: '700px' }}>
              ...
              {/* Map Legend */}
              <div className="absolute top-4 right-4 bg-white/95 backdrop-blur-sm rounded-lg shadow-lg border-2 border-gray-200 p-4 z-[1000] text-xs max-w-[220px]">
                <h4 className="font-bold text-gray-900 mb-3 flex items-center gap-2 text-sm border-b border-gray-200 pb-2">
              ...
              >
                ...
                {socialPosts.filter(post => post.isTrending).map(post => (
                  <React.Fragment key={`trending-${post.id}`}>
                    <Circle
                      center={[post.location.lat, post.location.lng]}
                      radius={post.likes * 100}
                      pathOptions={{
                        color: '#f59e0b',
                        fillColor: '#fbbf24',
                        fillOpacity: 0.2,
                    ...
                    <Marker position={[post.location.lat, post.location.lng]}>
                      <Popup>
                        <div className="p-2 min-w-[200px]">
                          <div className="flex items-center gap-2 mb-2">
                            <span className="text-lg">{post.nonprofitAvatar}</span>
                            <span className="text-xs font-bold text-orange-600">TRENDING</span>
                          </div>
                          <h3 className="font-bold text-sm text-gray-900">{post.nonprofitName}</h3>
                          <p className="text-xs text-gray-600 mt-1 line-clamp-3">{post.content}</p>
                          <div className="flex items-center gap-3 mt-2 text-xs text-gray-500">
                            <span>❤️ {post.likes}</span>
                            <span>💬 {post.comments}</span>
                            {post.awarenessCount && <span>📢 {post.awarenessCount} aware</span>}
                          </div>
                        </div>
                      </Popup>
                    </Marker>
        ...
        <div className="mt-8">
          <div className="flex items-center justify-between mb-6">
            ...
            <span className="text-sm text-gray-500">Latest from nonprofits you follow</span>
          </div>
              ...
              >
                ...
                <div className="p-4 border-b border-gray-100">
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3">
                      <div className="text-3xl">{post.nonprofitAvatar}</div>
                      <div>
                        <h3 className="font-bold text-sm text-gray-900">{post.nonprofitName}</h3>
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    {post.category === 'emergency' && <span className="text-xs bg-red-100 text-red-700 px-2 py-1 rounded-full font-semibold">🚨 Emergency</span>}
                    {post.category === 'milestone' && <span className="text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded-full font-semibold">🎯 Milestone</span>}
                    {post.category === 'success' && <span className="text-xs bg-green-100 text-green-700 px-2 py-1 rounded-full font-semibold">✨ Success</span>}
                    {post.category === 'update' && <span className="text-xs bg-gray-100 text-gray-700 px-2 py-1 rounded-full font-semibold">📋 Update</span>}
                    {post.category === 'announcement' && <span className="text-xs bg-purple-100 text-purple-700 px-2 py-1 rounded-full font-semibold">📢 Announcement</span>}
                    {post.isTrending && <span className="text-xs bg-orange-100 text-orange-700 px-2 py-1 rounded-full font-semibold">🔥 Trending</span>}
                  </div>

                  <p className="text-sm text-gray-700 leading-relaxed">{post.content}</p>

                  {/* Awareness Counter */}
                  {post.awarenessCount && post.awarenessCount > 0 && (
                ...
                <div className="px-4 pb-4 flex items-center justify-between border-t border-gray-100 pt-4">
                  <div className="flex items-center gap-4">
                    ...
                    >
                      <span className={`text-lg ${post.isLiked ? 'animate-bounce' : ''}`}>
                        {post.isLiked ? '❤️' : '🤍'}
                      </span>
                      <span className={`font-semibold ${post.isLiked ? 'text-red-600' : 'text-gray-600 group-hover:text-red-600'}`}>
                        {post.likes}
                      </span>
                    </button>
