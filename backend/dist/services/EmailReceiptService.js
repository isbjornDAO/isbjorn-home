...
class EmailReceiptService {
    ...
    constructor() {
        ...
        if (!this.isEnabled) {
            logger_1.logger.warn('⚠️ SendGrid not configured - email receipts disabled');
        }
        else {
            logger_1.logger.info('✅ Email receipt service initialized');
        }
    }
    /**
     * Generate IRD-compliant PDF receipt
     */
    async generatePDFReceipt(data) {
    ...
    async sendReceipt(data) {
        if (!this.isEnabled) {
            logger_1.logger.warn('SendGrid not enabled, skipping email send');
            return false;
        }
        try {
            logger_1.logger.info(`📧 Sending receipt email to ${data.donorEmail}`);
            ...
            // Send emails
            const emailPromises = recipients.map(email => this.sendSingleEmail(email, data, emailHTML, emailText, pdfBuffer));
            await Promise.all(emailPromises);
            logger_1.logger.info(`✅ Receipt emails sent successfully to ${recipients.join(', ')}`);
            return true;
        }
        catch (error) {
            logger_1.logger.error('❌ Failed to send receipt email:', error);
            return false;
        }
    }
    /**
     ...
     */
    async sendSingleEmail(email, data, html, text, pdfBuffer) {
        const msg = {
            to: email,
            from: {
    ...
    async testConfiguration() {
        ...
        try {
            ...
            await mail_1.default.send(msg);
            logger_1.logger.info('✅ Email test successful');
            return true;
        }
        catch (error) {
...
exports.default = new EmailReceiptService();
//# sourceMappingURL=EmailReceiptService.js.map