...
class AvalancheL1Service {
    ...
    isEnabled;
    constructor() {
        this.isEnabled = !!(process.env.AVALANCHE_RPC_URL &&
            process.env.AVALANCHE_PRIVATE_KEY &&
            process.env.AVALANCHE_CONTRACT_ADDRESS);
        if (this.isEnabled) {
            try {
                // Initialize provider
                this.provider = new ethers_1.JsonRpcProvider(process.env.AVALANCHE_RPC_URL);
                // Initialize wallet
                this.wallet = new ethers_1.Wallet(process.env.AVALANCHE_PRIVATE_KEY, this.provider);
                // Initialize contract
                this.contract = new ethers_1.Contract(process.env.AVALANCHE_CONTRACT_ADDRESS, DONATION_CONTRACT_ABI, this.wallet);
                logger_1.logger.info('✅ Avalanche L1 service initialized successfully');
            }
            catch (error) {
                logger_1.logger.error('❌ Failed to initialize Avalanche L1 service:', error);
                this.isEnabled = false;
            }
        }
        else {
            logger_1.logger.info('⚠️ Avalanche L1 service disabled - missing configuration');
        }
    }
    /**
     * Record a donation on the Avalanche L1 blockchain
     */
    async recordDonation(donationId, donorAddress, charityAddress, amountInWei, receiptHash) {
        if (!this.isEnabled) {
            logger_1.logger.warn('Avalanche L1 not enabled, skipping blockchain recording');
            return null;
        }
        try {
            logger_1.logger.info(`🔗 Recording donation ${donationId} on Avalanche L1...`);
            const tx = await this.contract.recordDonation(donationId, donorAddress, charityAddress, amountInWei, receiptHash, {
                gasLimit: 100000,
                gasPrice: process.env.AVALANCHE_GAS_PRICE ?
                    ethers_1.ethers.parseUnits(process.env.AVALANCHE_GAS_PRICE, 'wei') :
                    undefined
            });
            ...
            logger_1.logger.error(`❌ Failed to record donation on blockchain:`, error);
            throw error;
        }
    }
    /**
    ...
    async getDonation(donationId) {
        ...
        try {
            ...
            logger_1.logger.error(`Failed to get donation from blockchain:`, error);
            return null;
        }
    }
    /**
    ...
    async getTotalDonations() {
        ...
        try {
            ...
            logger_1.logger.error('Failed to get total donations from blockchain:', error);
            return 0;
        }
    }
    /**
     ...
     */
    async getCharityDonations(charityAddress) {
        if (!this.isEnabled)
            return 0;
        try {
            ...
            logger_1.logger.error('Failed to get charity donations from blockchain:', error);
            return 0;
        }
    }
    /**
    ...
    async healthCheck() {
        ...
        try {
            ...
            logger_1.logger.error('Avalanche L1 health check failed:', error);
            return false;
        }
    }
    /**
     ...
     */
    nzdToWei(nzdAmount, avaxPriceInNZD = 50) {
        const avaxAmount = nzdAmount / avaxPriceInNZD;
        return ethers_1.ethers.parseEther(avaxAmount.toString());
    }
    /**
     * Generate charity address from charity ID (deterministic)
     */
    generateCharityAddress(charityId) {
        const hash = ethers_1.ethers.keccak256(ethers_1.ethers.toUtf8Bytes(charityId));
        return ethers_1.ethers.getAddress('0x' + hash.slice(-40));
    }
    /**
     * Generate donor address from business info (deterministic)
     */
    generateDonorAddress(companyNumber, email) {
        const combined = `${companyNumber}_${email}`;
        const hash = ethers_1.ethers.keccak256(ethers_1.ethers.toUtf8Bytes(combined));
        return ethers_1.ethers.getAddress('0x' + hash.slice(-40));
    }
}
exports.default = new AvalancheL1Service();
//# sourceMappingURL=AvalancheL1Service.js.map