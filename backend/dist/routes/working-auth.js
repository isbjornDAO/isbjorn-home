...
router.post('/refresh', async (req, res) => {
    try {
        ...
        let decoded;
        try {
            decoded = jsonwebtoken_1.default.verify(refreshToken, process.env.JWT_REFRESH_SECRET || 'dev-refresh-secret');
        }
        catch (verifyError) {
            logger_1.logger.warn('Refresh token verification failed:', verifyError.message);
            return res.status(401).json({
                success: false,
        ...
        const token = jsonwebtoken_1.default.sign({ id: user.dataValues.id, email: user.dataValues.email, role: user.dataValues.role }, process.env.JWT_SECRET || 'dev-secret-key-change-in-production', { expiresIn: '7d' });
        logger_1.logger.info(`Token refreshed for user: ${user.dataValues.id}`);
        res.json({
            success: true,
            token
        });
