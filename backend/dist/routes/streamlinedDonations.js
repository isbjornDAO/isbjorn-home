...
const streamlinedDonationController_1 = __importDefault(require("../controllers/streamlinedDonationController"));
const rateLimiter_1 = require("../middleware/rateLimiter");
const router = express_1.default.Router();
const controller = new streamlinedDonationController_1.default();
// Validation rules
const donationValidation = [
    (0, express_validator_1.body)('nzCompanyNumber')
        .isString()
        .isLength({ min: 1, max: 10 })
        .matches(/^\d+$/)
        .withMessage('Valid NZ company number required'),
    (0, express_validator_1.body)('charityId')
        .isUUID()
        .withMessage('Valid charity ID required'),
    (0, express_validator_1.body)('amount')
        .isFloat({ min: 1 })
        .withMessage('Amount must be at least $1'),
    (0, express_validator_1.body)('stripePaymentMethodId')
        .isString()
        .notEmpty()
        .withMessage('Stripe payment method required'),
    (0, express_validator_1.body)('companyContactEmail')
        .isEmail()
        .withMessage('Valid contact email required'),
    (0, express_validator_1.body)('accountantEmail')
        .optional()
        .isEmail()
...
const companyNumberValidation = [
    (0, express_validator_1.param)('companyNumber')
        .isString()
        .isLength({ min: 1, max: 10 })
        .matches(/^\d+$/)
        .withMessage('Valid NZ company number required'),
 ...
 */
router.post('/streamlined', (0, rateLimiter_1.rateLimitMiddleware)({ windowMs: 60000, max: 5 }), // 5 donations per minute
donationValidation, controller.processDonation);
/**
 * @swagger
 ...
 */
router.get('/companies/:companyNumber/auto-populate', (0, rateLimiter_1.rateLimitMiddleware)({ windowMs: 60000, max: 20 }), // 20 lookups per minute
 ...
 */
router.get('/charities/search', (0, rateLimiter_1.rateLimitMiddleware)({ windowMs: 60000, max: 60 }), // 60 searches per minute
searchValidation, controller.searchCharities);
/**
 * @swagger
 ...
 *                       type: number
 */
router.get('/companies/:companyNumber/donations', companyNumberValidation, controller.getCompanyDonations);
/**
 * @swagger
 ...
 *                         type: string
 */
router.get('/companies/:companyNumber/compliance-dashboard', companyNumberValidation, controller.getComplianceDashboard);
/**
 * @swagger
 ...
 */
router.get('/receipts/:donationId/download', (0, express_validator_1.param)('donationId').isString().notEmpty(), controller.downloadReceipt);
exports.default = router;
//# sourceMappingURL=streamlinedDonations.js.map