...
class DatabaseSeeder {
    async seedUsers() {
        try {
            const existingUsers = await User_model_1.default.count();
            if (existingUsers > 0) {
                logger_1.logger.info('Users already exist, skipping user seeding');
                return;
            }
            // Create admin user
    ...
    async seedCharities() {
        try {
            const existingCharities = await Charity_model_1.default.count();
            if (existingCharities > 0) {
                logger_1.logger.info('Charities already exist, skipping charity seeding');
                return;
            }
            const charities = [
                {
            ...
            await Charity_model_1.default.bulkCreate(charities);
            logger_1.logger.info('✅ Charities seeded successfully');
        }
        catch (error) {
            logger_1.logger.error('❌ Failed to seed charities:', error);
            throw error;
        }
    }
    async seedProjects() {
        try {
            const existingProjects = await Project_model_1.default.count();
            if (existingProjects > 0) {
                logger_1.logger.info('Projects already exist, skipping project seeding');
                return;
            }
            ...
            logger_1.logger.info('✅ Projects seeded successfully');
        }
        catch (error) {
            logger_1.logger.error('❌ Failed to seed projects:', error);
            throw error;
        }
    }
    async seedAll() {
