...
interface IRDReceiptData {
  // ALL 7 IRD required elements per IRD255 guidance
...
export class IRDReceiptService {
  private emailTransporter: nodemailer.Transporter;
  private receiptsDirectory: string;
  private templatePath: string;

  constructor() {
    this.receiptsDirectory = path.join(process.cwd(), 'storage', 'receipts');
    this.templatePath = path.join(process.cwd(), 'assets', 'receipt-templates');
   ...
   */
  async generateIRDCompliantReceipt(donationId: string): Promise<{
    pdfPath: string;
    receiptNumber: string;
    ...
    try {
      ...
      const receiptData: IRDReceiptData = {
        organisationLogo: path.join(this.templatePath, 'isbjorn-official-logo.png'),
        ...
        authorisedPerson: {
          ...
          digitalSignature: path.join(this.templatePath, 'treasurer-signature.png'),
        },
      };
      ...
      logger.info(`IRD-compliant receipt generated in ${generationTime}ms`, {
        donationId,
        receiptNumber: donation.receiptNumber,
      ...
      const generationTime = Date.now() - startTime;
      logger.error(`Receipt generation failed after ${generationTime}ms:`, {
        donationId,
        error: error.message,
      });
   ...
   */
  async emailReceipt(donationId: string, recipients: {
    donor: string;
    accountant?: string;
    cc?: string[];
  }): Promise<{
    ...
    try {
      ...
      if (recipients.accountant) allRecipients.push(recipients.accountant);
      if (recipients.cc) allRecipients.push(...recipients.cc);
      ...
      logger.info('IRD receipt email sent successfully', {
        donationId,
        receiptNumber: donation.receiptNumber,
   ...
   */
  async generateSequentialReceiptNumber(): Promise<string> {
  ...
  // Private methods
  private async createPDFReceipt(data: IRDReceiptData): Promise<string> {
    const fileName = `receipt-${data.receiptNumber}.pdf`;
    const filePath = path.join(this.receiptsDirectory, fileName);
    ...
    if (data.organisationLogo && fs.existsSync(data.organisationLogo)) {
    ...
    if (data.authorisedPerson.digitalSignature && fs.existsSync(data.authorisedPerson.digitalSignature)) {
  ...
  private generateEmailBody(donation: IRDCompliantDonation): string {
    return `
      <html>
